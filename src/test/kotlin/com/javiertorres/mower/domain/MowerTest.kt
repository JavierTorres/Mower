...
internal class MowerTest {
    @Test
    fun `Given a command returning a new position When moving Then it returns the last ok position`() {
        ...
        assertEquals(
            finalPosition,
            Mower(listOf(command), Position(Coordinate(3, 3), Direction.W)).move())
    }
}